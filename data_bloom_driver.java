...
public class bloom_driver{
	...
	public static void main(String[] args) throws IOException {
		...
		PrintWriter toList = new PrintWriter(new FileWriter("list.txt"));
		...
		bp_count = total_pos_length + total_neg_length;
		System.out.println("bp_count: " + bp_count);
		if(normalize)
		...
		{//for test portion of the reads
			...
			{//for each character make kmer of kmer_size
				...
				if(bloom_filter_neg.contains(kmer))
				{
					...
					if(!bloom_filter_pos.contains(kmer))//kmer - but not +
					{
						toList.println(kmer);
					}
				}
